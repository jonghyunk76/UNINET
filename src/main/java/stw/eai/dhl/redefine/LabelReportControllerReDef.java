/**
 * @author ally
 * This Class ReDefine com.dhl.sop.label.LabelReportController
 * Modify Listing
 * 1. JRLoader.loadObject(path) relative path => absolute path 
...
import com.dhl.sop.label.InternalServiceCodeComparator;

public class LabelReportControllerReDef {
...
//    	System.out.println("LabelReportController: Inside method generateSOPLabel");
		...
		jasperReport = (JasperReport) JRLoader.loadObjectFromFile(Common.JASPER_PATH + "JasperReports\\SOPLabel.jasper");//End
		JasperPrint jasperPrint;
		
		//Start:Populating HashMap with label values which are same for all the pieces (pages)
	    Map<String, Object> shipmentDetails = new HashMap<String, Object>();
		...
		Collection<Map<String, ?>> pieceFieldMaps = new ArrayList<Map<String,?>>();
		...
		while(itrPiece.hasNext())	{
			...
			String licensePlateWithSpaces = ArchiveLabelReportControllerReDef.getLicensePlateWithSpaces(shipValResponsePiece.getLicensePlate());
