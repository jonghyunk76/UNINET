...
(function(L) {
	...
	ag.createDate = function(aj) {
		...
		// 날짜 타입과 맞지 않는 경우, 자동으로 날짜 타입으로 변경한다.(김종현, 2015.07.17)
		// 단, DB에 저장된 형태로 정수형태의 값만 처리가능함
		if(ao.length < 8) {
			if(ao.length == 4) ao = ao+"-01-01"; // 년단위
			if(ao.length == 6) {
				ao = ao.substring(0, 4)+"-"+ao.substring(4, 6)+"-01";    // 년월단위
			}
		} else {
			if(ao.length == 8) {
				ao = ao.substring(0, 4)+"-"+ao.substring(4, 6)+"-"+ao.substring(6, 8);    // 년월단위
			}
		}
