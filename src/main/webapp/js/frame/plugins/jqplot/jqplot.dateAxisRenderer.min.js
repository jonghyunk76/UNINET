			...
			if (this.min == null && this.max == null && H.min == H.max) {
				...
				if (this.min == null && this.max == null) {
					...
					if (I <= 19 * l) {
						...
						ae = new h.jsDate(ae);
						ae = Math.floor((ae.getTime() - ae.getUtcOffset()) / r)
								* r + ae.getUtcOffset();
						...
						if (I <= 9 * j) {
							...
							var V = new h.jsDate(ae).setDate(1).setHours(0, 0,
									0, 0);
							...
							ab = Math.ceil(S / D) + 1;
							this.min = V.getTime();
							this.max = V.clone().add((ab - 1) * D, "month")
									.getTime();
							this.numberTicks = ab;
							for (var aa = 0; aa < ab; aa++) {
								if (aa === 0) {
							...
							var V = new h.jsDate(ae).setMonth(0, 1).setHours(0,
									0, 0, 0);
							...
							ab = Math.ceil(K / D) + 1;
							this.min = V.getTime();
							this.max = V.clone().add((ab - 1) * D, "year")
									.getTime();
							this.numberTicks = ab;
							for (var aa = 0; aa < ab; aa++) {
								if (aa === 0) {
					...
					var W, C;
					W = (this.min != null) ? new h.jsDate(this.min).getTime()
					...
					for (var aa = 0; aa < this.numberTicks; aa++) {
						...
						ad = ae.add(aa * this.daTickInterval[0],
								this.daTickInterval[1]).getTime();
						var Y = new this.tickRenderer(this.tickOptions);
						if (!this.showTicks) {
							Y.showLabel = false;
