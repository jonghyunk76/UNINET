...
var groupview = $.extend({}, $.fn.datagrid.defaults.view, {
	render: function(target, container, frozen){
		...
		if(groups.length > 0) {
			...
			var hwsize = (opts.headerWidthSize > 0)?opts.headerWidthSize:10000;
			table.push("<table style=\"width:"+hwsize+"px;height:1px;\" border=\"0\">"); // 값이 없는 경우에는 전체 해더를 보여주기 위해 임의로 table를 생성함(2021.11.19)
		}
...
//				console.log("btnclick ==> " + gid + ", " + gindex);
				status.gid = gid;
				status.index = gindex;
				...
				grid.setGroupConfig(gid, status); // 그룹뷰 상태를 유지하기 위해 상태값을 저장한다.(2020-12-29)
			} else {
				clickHandler(e);
			}
			// e.stopPropagation();
