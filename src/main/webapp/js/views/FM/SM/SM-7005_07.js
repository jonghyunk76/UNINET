/******************************************************************************************
 * 작성자 : YNI-Maker
 * Program Id : SM-7005_07
 * 작업일자 : 2022.07.29
 * 
 ******************************************************************************************/

function onLoadPage() {
    SM7005_07.init.initComponent();
}
...
var SM7005_07 = {
    ...
    init : {
        initComponent : function() {
            ...
            var vFlag = nullToString(form.handle.getValue("SM7005_07_form_01", "flag"));

            // 초기화면에 보여줄 데이터 조회
            if (vFlag.indexOf("insert") == -1) {
                SM7005_07.control.selectInterfaceItemMstInfo();
            }
            
            SM7005_07.calendar.initCalendar_1();
            SM7005_07.combobox.initCombo_1();
            SM7005_07.datagrid.initGrid_1();
            
            // ui변경
            if (vFlag == "view") {
                SM7005_07.ui.viewChangeUI();
            } else if (vFlag == "update") {
                SM7005_07.ui.updateChangeUI();
            } else {
                SM7005_07.ui.insertChangeUI();
            }
        }
    }, 
    // 달력 그리드 생성
    ...
    combobox : {
        ...
        initCombo_1 : function() {
            var vCompanyCd = form.handle.getValue("SM7005_07_form_01", "COMPANY_CD") 
            
            var objUsingYn = combo.getObject("SM7005_07_form_01", "USING_YN");
    ...
    event : {
        ...
        selectAfterUI : function(data) {
            ...
            form.handle.loadData("SM7005_07_form_01", data);
        },
        ...
        updateAfterUI : function(data) {
            ...
            if (vFlag.indexOf("insert") > -1) {
                // update 상태로 변경한다.
                form.handle.setValue("SM7005_07_form_01", "flag", "update");
                
                form.handle.setValue("SM7005_07_form_01", "INTERFACE_ITEM_MST_ID", oMap.INTERFACE_ITEM_MST_ID);
            }
            ...
            //var dl_1 = dialog.getObject("SM7005_07_dailog_01");
    ...
    control : {
        selectInterfaceItemMstInfo : function() {
            var obj = form.getObject("SM7005_07_form_01");
        ...
        updateInterfaceItemMstInfo : function() {
            ...
            if (!form.handle.isValidate("SM7005_07_form_01")) return;
            var confMsg = "";
        ...
        validate : function(pType) {
            ...
            var vCompanyCd = form.handle.getValue("SM7005_07_form_01", "COMPANY_CD");
            var vIfCode = form.handle.getValue("SM7005_07_form_01", "IF_CD");
    ...
    dialog : {
        ...
        openDialog : function(flag) {
            var dg_1 = grid.getObject("SM7005_07_grid_01");
            ...
            // 팝업으로 넘겨줄 파라메터 생성
            var params = {};
            var params1 = form.handle.getElementsParam("SM7005_07_form_01");
            var params2 = {};
    ...
    ui : {
        insertChangeUI : function() {
            ...
            // 삭제버튼감춤
            form.util.setVisible("SM7005_07_btn_02", false);
        },
        updateChangeUI : function() {
            
            // 삭제버튼보임
            form.util.setVisible("SM7005_07_btn_02", true);
            
            // key가 되는 input은 모두 막는다.
            form.handle.readonly("SM7005_07_form_01", "IF_CD", true);
            form.handle.addClass("SM7005_07_form_01", "IF_CD", "input_readOnly");
        },
        viewChangeUI : function() {
            
            dialog.handle.resize("SM7005_07_dailog_01", 600, 225);
            
            // 버튼감싸는영역
            form.util.setVisible("SM7005_07_div_01", false);
            
            // form안 모든 element에 대해서 label과 같은 형식으로 표시한다.
            form.handle.closeAll("SM7005_07_form_01");
        }
    },
    // 파일 input 엘리먼트 구현
    file : {
