/******************************************************************************************
 * 작성자 : 유동기
 * Program Id : SM-8002_02
 * 작업일자 : 2017.07.12
 * 
 ******************************************************************************************/

function onLoadPage() {
    SM8002_02.config.applyFtaNation();
    SM8002_02.init.initComponent();
}
...
var SM8002_02 = {
    ...
    init : {
        initComponent : function() {
            var flag = form.handle.getValue("SM8002_02_form_01", "flag");
            
            SM8002_02.datagrid.initGrid_1();
            
            if(flag != "insert"){
                SM8002_02.ui.actionChangeUI();
            }
            
            SM8002_02.tooltip.makeTooltip();
        }
    }, 
    // 달력 생성
    calendar : {}, 
    ...
    tooltip : {
        makeTooltip : function() {
            form.util.setFileTooltip("SM8002_02_files_1", "SM8002_02_grid_02", null, 500, 100);
        },
        showTooltip : function() {
            form.util.showTooltip("SM8002_02_files_1");
        },
        hideTooltip : function() {
            form.util.hideTooltip("SM8002_02_files_1");
        },
        setFiles : function(pfile) {
        	...
        	setTimeout( function() { // 크롭에서 문제가 있어 0.5정도 후에 보이도록 수정(2020-05-14)
	            ...
	            form.handle.setValue("SM8002_02_form_01", "file_aliase", msg);
	            form.handle.setValue("SM8002_02_form_01", "FILE_SIZE", tts);
	            
	            SM8002_02.datagrid.initGrid_2(fileArray);
        	}, 500);
        }
    },
    // 데이터 그리드 생성
    datagrid : {
        header1 : [
               ["ORIGIN_FILE_NAME"      ,resource.getMessage("FILE, LIST")             ,300    ,"left"     ,"center"   ,false    ,false    ,null        ,null    ,{format: 'function', pid:'SM8002_02', name:'fileDownloadLink'}    ,0        ,0 ],
               ["FILE_SEQ"              ,resource.getMessage("DEL")                    ,80     ,"center"   ,"center"   ,true     ,false    ,null        ,null    ,{format: 'function', pid:'SM8002_02', name:'formatDelButton'}    ,0        ,0]
         ],
         header2 : [
         ...
         initGrid_1 : function() {
        	 var flag = form.handle.getValue("SM8002_02_form_01", "flag");
        	 var params = form.handle.getElementsParam("SM8002_02_form_02");
             var dg_1 = grid.getObject("SM8002_02_grid_01");
         ...
         initGrid_2 : function(fileArray) {
             var dg_1 = grid.getObject("SM8002_02_grid_02");
    ...
    event : {
        updateAfterUI : function(datas) {
        	form.handle.setValue("SM8002_02_form_01", "flag", "update");
        	form.handle.setValue("SM8002_02_form_01", "FILE_IN_FLAG", datas.dataMap.map.file_aliase);
        	
        	if(!oUtil.isNull(datas.BOARD_ID)) {
        		form.handle.setValue("SM8002_02_form_01", "BOARD_ID", datas.BOARD_ID);
        		form.handle.setValue("SM8002_02_form_02", "BOARD_ID", datas.BOARD_ID);
        		form.handle.setValue("SM8002_02_form_01", "BOARD_NO", datas.BOARD_NO);
        		form.handle.setValue("SM8002_02_form_01", "CREATE_DATE", datas.CREATE_DATE);
        		form.handle.setValue("SM8002_02_form_01", "READ_COUNT", datas.READ_COUNT);
            }
        	
        	SM8002_02.control.selectQnAFile();
        	SM8002_01.control.selectQnAList();
        	SM8002_02.ui.actionChangeUI();
        },
        deleteAfterUI : function() {
            var dl_1 = dialog.getObject("SM8002_02_dialog_01");
        ...
        readCountSetUI : function(data) {
        	form.handle.setValue("SM8002_02_form_01", "READ_COUNT", data.READ_COUNT);
        },
        readCountUI : function() {
        	SM8002_02.control.selectReadCount();
        },
        searchQnAFile : function() {
        	var fileCount = form.handle.getValue("SM8002_02_form_01", "FILE_COUNT");
        	var fileInFlag = form.handle.getValue("SM8002_02_form_01", "FILE_IN_FLAG");
        	var dg_1 = grid.getObject("SM8002_02_grid_02");
        	
        	if((!oUtil.isNull(fileCount) && fileCount > 0) || !oUtil.isNull(fileInFlag)) {
        		form.handle.setValue("SM8002_02_form_01", "file_aliase","");
    ...
    control : {
        ...
        updateQnAInfo : function() {
            if(!form.handle.isValidate("SM8002_02_form_01")) return;
            
            var confMsg = "";
            if(form.handle.getValue("SM8002_02_form_01", "flag") == "update") {
            ...
            $.messager.confirm(resource.getMessage("CNFIR"), confMsg, function(flag) {
                if(flag) {
                    var obj = form.getObject("SM8002_02_form_01");
        ...
        selectReadCount : function() {
            var obj = form.getObject("SM8002_02_form_02");
        ...
        downloadConfirmFile : function() {
            ...
            form.handle.setValue("SM8002_02_form_01", "BOARD_FILE_ID", rowData.FILE_SEQ);
        ...
        insertReplyInfo : function(flag) {
        	var boardId = form.handle.getValue("SM8002_02_form_01", "BOARD_ID");
        	var content = form.handle.getValue("SM8002_02_form_01", "CONTENT");
        	var params = new Object();
        	
        	params.PARENT_BOARD_ID = boardId;
        	params.CONTENT = content;
        	params.flag = flag;
        	SM8002_01.dialog.openDialog_1(params);
        	
        	form.handle.reset("SM8002_02_form_01");
        }
    },
    // 다이얼로그 구현
    dialog : {},
    ...
    ui : {
        fileDownloadLink : function(value, row, index) {
            return "<a href=\"javascript:SM8002_02.control.downloadConfirmFile();\" title=\"" + value + "\" class=\"easyui-tooltip\">"+
                   "<img src=\"/images/icon/external_link.png\" boder=\"0\"/> " + value + "</a>";
        },
        formatDelButton : function(value, row, index) {
        	var createBy = form.handle.getValue("SM8002_02_form_01","CREATE_BY");
            var userId = form.handle.getValue("SM8002_02_form_01","SESSION_USER_ID");

            if(!oUtil.isNull(value) && createBy ==  userId) {
                return "<a href='javascript:SM8002_02.control.fncDeleteFile();' class='btn'>"+resource.getMessage("DEL")+"</a>";
            } else {
                return "";
            }
        },
        actionChangeUI : function() {
            var flag = form.handle.getValue("SM8002_02_form_01", "flag");
            var createBy = form.handle.getValue("SM8002_02_form_01","CREATE_BY");
            var userId = form.handle.getValue("SM8002_02_form_01","USER_ID");
            
            if(flag == "insert") {
                form.util.setVisible("SM8002_02_btn_div_1", true);
            } else if(flag == "update") {
                if(userId != createBy){
	                form.handle.closeAll("SM8002_02_form_01");
	                
	                form.util.setVisible("SM8002_02_btn_02", false);
	                form.util.setVisible("SM8002_02_btn_div_1", false);
	                form.util.setVisible("SM8002_02_files_div", false);
	                form.util.setVisible("SM8002_02_include_div", false);
                } else {
	                form.util.setVisible("SM8002_02_btn_03", true);
                }
            }
        }
    }, 
